...
STM_LIFETIME = 30.0
WM_SLOTS = 7
# Repaired: old value decayed episodic memory in minutes. Retain for years by default.
EPISODIC_RETENTION_SECONDS = 730 * 24 * 60 * 60
...
class MemoryManager:
    ...
    def episodic_recall(self, query: Dict[str, Any], top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        for mem in self._episodic:
            ...
            decay = max(0.05, 1.0 - min(1.0, age / EPISODIC_RETENTION_SECONDS))
    ...
    def episodic_apply_decay(self) -> int:
        ...
        for mem in self._episodic:
            age = now - mem["timestamp"]
            decay = max(0.0, 1.0 - min(1.0, age / EPISODIC_RETENTION_SECONDS))
            if decay <= 0.0:
...
class MemoryModule(CognitiveModule):
    ...
    async def _handle_request(self, data: Dict[str, Any]) -> None:
        ...
        if query_type == "stm":
            ...
            matches = self._manager.semantic_query_similar(subject)
            result = {"semantic_matches": matches, "items": matches}
        ...
        if self._kernel is not None:
            self._kernel.event_bus.emit(
                Event(type="memory_retrieved", data=result, source_module=self.module_id),
                Priority.COGNITIVE,
            )
