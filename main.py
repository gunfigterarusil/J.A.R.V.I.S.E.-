...
async def run_with_chat(kernel: Kernel) -> None:
    ...
    print("Jarvis chat mode. Type /help for commands. Common: /status, /models, /system, /memory, /recall, /see, /gui, /task, /skills, /actions, /repair, /safety, /approve, /exit. Natural language works too.\n")
    try:
        while kernel.running or not kernel_task.done():
            ...
            if lower in {"/status", "/state"}:
                ...
                print(f"- modules: {len(kernel.modules)}")
                print(f"- data: {getattr(kernel.config, 'persistence_dir', '')}")
                actions_cfg = getattr(kernel.config, 'actions', None)
                print(f"- workspace: {getattr(actions_cfg, 'workspace_path', '') if actions_cfg else ''}")
                print(f"- safety level: L{int(kernel.permission_manager.current_level)} {kernel.permission_manager.current_level.name}")
                print(f"- providers: {', '.join(kernel.llm_router.available_providers())}\n")
                continue

            if lower in {"/modules", "/module-status"}:
                print("Loaded modules:")
                for name in sorted(kernel.modules.keys()):
                    print(f"- {name}")
                print()
                continue

            if lower in {"/events", "/event-stats"}:
